...
const defaultQueryParams= (pageSize: number, searchNameTerm?: string): string => {
  return searchNameTerm
  ? `?pageNumber=1&pageSize=${pageSize}&sortBy=name&sortDirection=asc&searchNameTerm=${searchNameTerm}`
  : `?pageNumber=1&pageSize=${pageSize}&sortBy=name&sortDirection=asc`
}
  // --- GET --- //
  it('get /blogs', async () => {
    await blogCollection.drop()
    await blogCollection.insertMany(createBlogs(12))
    const pageSize = 5;

    const res = await req
      .get(SETTINGS.PATH.BLOGS + defaultQueryParams(pageSize, '1'))
      .expect(200)
      ...
      expect(res.body.items[0].name).toEqual('name1')
      expect(res.body.items[3].name).toEqual('name12')
  })
  //  
  //
